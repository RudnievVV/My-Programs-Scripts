...
while True:
    ...
    for student in range(1, len(list_of_students) - 1):
        if list_of_students[student][0] == list_of_students[student + 1][0] or list_of_students[student][0] == list_of_students[student - 1][0]:
            try_count = try_count + 1
            random.shuffle(list_of_students)
            break
        else:
...
while True:
    ...
    if nums[0] == nums[1] + 1 or nums[len(nums) - 1] == nums[len(nums) - 2] - 1:
    ...
    for student_num in range(1, len(nums) - 1):
        if nums[student_num] + 1 == nums[student_num + 1] or nums[student_num] - 1 == nums[student_num + 1]:
            try_count_nums = try_count_nums + 1
            random.shuffle(nums)
            break
        else:
